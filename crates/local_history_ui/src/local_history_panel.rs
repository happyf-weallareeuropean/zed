...
use gpui::{
    ...
    ListAlignment, ListState, ModifiersChangedEvent, MouseButton, MouseDownEvent, Pixels, Point,
    Render, SharedString, Subscription, Task, WeakEntity, Window, actions, anchored, black,
    deferred, div, list, px, transparent_black,
};
use language::Buffer;
use project::{
...
use ui::{
    ...
    ListItemSpacing, PopoverMenu, Tab, TintColor, Tooltip, h_flex, prelude::*, v_flex,
};
...
impl LocalHistoryPanel {
    ...
    fn active_project_abs_path(&self, cx: &App) -> Option<PathBuf> {
        let project_path = self.active_project_path.as_ref()?;
        let worktree = self
            .project
            .read(cx)
    ...
    fn toggle_entry_bookmark(&mut self, ix: usize, cx: &mut Context<Self>) {
        let Some(entry) = self.entries.get(ix).cloned() else {
            return;
        };
        let bookmarked = !entry.bookmarked;
        if let Some(current) = self.entries.get_mut(ix) {
            current.bookmarked = bookmarked;
        }
        let store = self.project.read(cx).local_history_store().clone();
        store.update(cx, |store, cx| {
            store.set_entry_bookmarked(entry, bookmarked, cx);
        });
        cx.notify();
    }

    fn open_entry_diff(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) {
        self.open_entry_view(ix, LocalHistoryViewMode::Diff, window, cx);
    }

    fn open_entry_reconstruct(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) {
        self.open_entry_view(ix, LocalHistoryViewMode::Reconstruct, window, cx);
    }

    fn open_entry_view(
        &mut self,
        ix: usize,
        mode: LocalHistoryViewMode,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(entry) = self.entries.get(ix).cloned() else {
        ...
        let opening_message = match mode {
            LocalHistoryViewMode::Diff => "Opening local history diff…",
            LocalHistoryViewMode::Reconstruct => "Opening local history reconstruct view…",
        };
        self.show_toast(opening_message, cx);
            ...
            if let Some(workspace) = workspace.upgrade() {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        ...
                        let title = SharedString::from(match mode {
                            LocalHistoryViewMode::Diff => format!("Local History: {file_name}"),
                            LocalHistoryViewMode::Reconstruct => {
                                format!("Reconstruct: {file_name}")
                            }
                        });
    ...
    fn deploy_context_menu(
        ...
        let reveal_path = self.active_project_abs_path(cx);
        let is_bookmarked = self.entries.get(ix).is_some_and(|entry| entry.bookmarked);

        let context_menu = ContextMenu::build(window, cx, move |menu, _, _| {
            let panel_for_open = panel.clone();
            let panel_for_reconstruct = panel.clone();
            let panel_for_restore = panel.clone();
            let panel_for_bookmark = panel.clone();
            menu.context(focus_handle.clone())
                .entry("Open Diff", None, move |window, cx| {
                    if let Some(panel) = panel_for_open.upgrade() {
                        panel.update(cx, |panel, cx| panel.open_entry_diff(ix, window, cx));
                    }
                })
                .entry("Open Reconstruct View", None, move |window, cx| {
                    if let Some(panel) = panel_for_reconstruct.upgrade() {
                        panel.update(cx, |panel, cx| panel.open_entry_reconstruct(ix, window, cx));
                    }
                })
                .entry("Restore Snapshot", None, move |window, cx| {
                    if let Some(panel) = panel_for_restore.upgrade() {
                        panel.update(cx, |panel, cx| panel.restore_entry(ix, window, cx));
                    }
                })
                .entry(
                    if is_bookmarked {
                    ...
                    None,
                    move |_, cx| {
                        if let Some(panel) = panel_for_bookmark.upgrade() {
                            panel.update(cx, |panel, cx| panel.toggle_entry_bookmark(ix, cx));
                        }
                    },
                )
                .when_some(reveal_path.clone(), |menu, reveal_path| {
                    ...
                    menu.separator().entry(label, None, move |_, cx| {
        ...
        let subscription = cx.subscribe(&context_menu, |panel, _, _: &DismissEvent, cx| {
    ...
    fn render_local_history_entry(
        ...
        let bookmark_color = cx.theme().colors().icon_accent;
        ...
        let toggle_bookmark = cx.listener(move |panel: &mut Self, _event: &ClickEvent, _, cx| {
            panel.toggle_entry_bookmark(ix, cx);
        });
        ...
        let item = ListItem::new(format!("local-history-entry-{}", entry.id))
            ...
            .end_slot(
                h_flex()
                    ...
                    .child(
                        IconButton::new(
                            format!("local-history-bookmark-{}", entry.id),
                            IconName::Star,
                        )
                        .icon_size(IconSize::XSmall)
                        .toggle_state(entry.bookmarked)
                        .selected_style(ButtonStyle::Tinted(TintColor::Accent))
                        .selected_icon_color(Color::Custom(bookmark_color))
                        .on_click(toggle_bookmark)
                        .tooltip(Tooltip::text("Toggle bookmark")),
                    )
                    .child(
                        IconButton::new(
                        ...
                        .visible_on_hover("local-history-entry-hover"),
                    ),
            )
            .child(
                h_flex().h_full().items_center().child(
        ...
        let item = div()
            ...
            .rounded_md()
            .border_1()
            .border_color(if entry.bookmarked {
                bookmark_color
            } else {
                transparent_black()
            })
            .bg(entry_bg)
            .group("local-history-entry-hover")
            .on_mouse_down(
                MouseButton::Right,
                cx.listener(move |panel, event: &MouseDownEvent, window, cx| {
                    cx.stop_propagation();
                    panel.deploy_context_menu(ix, event.position, window, cx);
                }),
            )
            .child(item);
        if f32::from(gap) > 0.0 {
...
fn render_local_history_hunk_controls(
    row: u32,
    _status: &buffer_diff::DiffHunkStatus,
    hunk_range: std::ops::Range<editor::Anchor>,
    ...
    h_flex()
        ...
        .shadow_md()
        .child(
            Button::new(("restore-local-history-hunk", row as u64), "Restore")
                .tooltip(Tooltip::text("Restore this hunk from the snapshot"))
                .on_click({
...
impl LocalHistoryDiffView {
    fn new(
        ...
        let editor = cx.new(|cx| {
            ...
            let controls: RenderDiffHunkControlsFn = match mode {
                LocalHistoryViewMode::Diff => {
                    Arc::new(|_, _, _, _, _, _, _, _| gpui::Empty.into_any_element())
                }
                LocalHistoryViewMode::Reconstruct => Arc::new(render_local_history_hunk_controls),
            };
    ...
    fn restore_snapshot(&mut self, cx: &mut Context<Self>) {
        let snapshot_text = self.snapshot_text.clone();
        self.buffer.update(cx, |buffer, cx| {
            buffer.set_text(snapshot_text, cx);
        });
    }

    fn restore_selected_hunks(&mut self, window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn toggle_bookmark(&mut self, cx: &mut Context<Self>) {
        let bookmarked = !self.entry.bookmarked;
        self.entry.bookmarked = bookmarked;
        let store = self.project.read(cx).local_history_store().clone();
    ...
    fn render_reconstruct_header(
        ...
        let storage_kind = match self.entry.storage_kind {
            project::LocalHistoryStorageKind::Snapshot => "Snapshot",
            project::LocalHistoryStorageKind::Delta => "Delta",
        };
        let metadata = if self.entry.base_entry_id.is_some() {
            format!(
                "{storage_kind} • chain {} • {} / {} bytes",
                self.entry.chain_depth, self.entry.compressed_bytes, self.entry.uncompressed_bytes
            )
        } else {
            format!(
                "{storage_kind} • {} / {} bytes",
                self.entry.compressed_bytes, self.entry.uncompressed_bytes
            )
        };
                ...
                h_flex()
                    ...
                    .child(
                        v_flex()
                            .gap_0p5()
                            .child(
                                Label::new("Reconstruct from this local-history entry")
                                    .size(LabelSize::Small),
                            )
                            .child(
                                Label::new(format!("{time} • {metadata}"))
                                    .size(LabelSize::XSmall)
                                    .color(Color::Muted),
                            )
                            .child(
                                Label::new(
                                    "Select diff hunks to restore part of the snapshot, or restore the full snapshot.",
                                )
                                .size(LabelSize::XSmall)
                                .color(Color::Muted),
                            ),
                    ...
                    .child(
                        h_flex()
                            ...
                            .gap_1()
                            .child(
                                IconButton::new("local-history-view-bookmark", IconName::Star)
                                    .icon_size(IconSize::Small)
                                    .toggle_state(self.entry.bookmarked)
                                    .selected_style(ButtonStyle::Tinted(TintColor::Accent))
                                    .selected_icon_color(Color::Accent)
                                    .on_click(cx.listener(
                                        |view: &mut Self, _event: &ClickEvent, _window, cx| {
                                            view.toggle_bookmark(cx);
                                        },
                                    ))
                                    .tooltip(Tooltip::text("Toggle bookmark")),
                            )
                            .child(
                                Button::new("local-history-restore-selected", "Restore Selected")
                                    .tooltip({
                                    ...
                                    .on_click(cx.listener(
                                        |view: &mut Self,
                                         ...
                                         window,
                                         cx| {
                                            view.restore_selected_hunks(window, cx);
                                        },
                                    )),
                            )
                            .child(
                                Button::new("local-history-restore-all", "Restore Snapshot")
                                    .on_click(cx.listener(
                                        |view: &mut Self, _event: &ClickEvent, _window, cx| {
                                            view.restore_snapshot(cx);
                                        },
                                    )),
                            ),
                    ),
...
impl Item for LocalHistoryDiffView {
    ...
    fn telemetry_event_text(&self) -> Option<&'static str> {
        Some(match self.mode {
            LocalHistoryViewMode::Diff => "Local History Diff Opened",
            LocalHistoryViewMode::Reconstruct => "Local History Reconstruct Opened",
        })
    }
...
impl Render for LocalHistoryDiffView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        match self.mode {
            LocalHistoryViewMode::Diff => self.editor.clone().into_any_element(),
            LocalHistoryViewMode::Reconstruct => v_flex()
                .size_full()
                .child(self.render_reconstruct_header(window, cx))
...
mod tests {
    ...
    async fn test_open_diff_from_local_history_entry(cx: &mut TestAppContext) {
        ...
        let entry = LocalHistoryEntry {
            ...
            bookmarked: false,
            storage_kind: project::LocalHistoryStorageKind::Snapshot,
            base_entry_id: None,
            chain_depth: 0,
        };
    ...
    #[gpui::test]
    async fn test_open_reconstruct_from_local_history_entry(cx: &mut TestAppContext) {
        init_test(cx);
        ...
        let snapshot_text = "old text\n";
        let compressed = zstd::encode_all(snapshot_text.as_bytes(), 0).unwrap();
        let snapshot_rel = PathBuf::from("snapshots/reconstruct.zst");
        std::fs::create_dir_all(history_root.join("snapshots")).unwrap();
        std::fs::write(history_root.join(&snapshot_rel), compressed).unwrap();
        ...
        cx.update(|window, cx| {
            panel.update(cx, |panel, cx| panel.open_entry_reconstruct(0, window, cx));
        });

        cx.run_until_parked();

        let reconstruct_view = workspace.read_with(cx, |workspace, cx| {
            workspace
                .items_of_type::<LocalHistoryDiffView>(cx)
                .next()
                .unwrap()
        });
        assert_eq!(
            reconstruct_view.read_with(cx, |view, _| view.mode),
            LocalHistoryViewMode::Reconstruct
        );
    }
