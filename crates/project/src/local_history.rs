...
const MAX_DELTA_CHAIN_DEPTH: u16 = 32;
...
pub struct LocalHistoryEntry {
    ...
    pub bookmarked: bool,
    pub storage_kind: LocalHistoryStorageKind,
...
impl LocalHistoryEntry {
    ...
    pub fn worktree_hash(&self) -> Option<String> {
        extract_worktree_hash(&self.snapshot_relative_path)
    }
...
impl Settings for LocalHistorySettings {
    fn from_settings(content: &settings::SettingsContent) -> Self {
        ...
        Self {
            enabled: local_history.enabled.unwrap_or(defaults.enabled),
            capture_on_save: local_history
                .capture_on_save
                .unwrap_or(defaults.capture_on_save),
            capture_on_edit_idle_ms: local_history
                .capture_on_edit_idle_ms
                .map(|delay| delay.0)
                .or(defaults.capture_on_edit_idle_ms)
                .filter(|delay| *delay > 0),
            capture_on_focus_change: local_history
                .capture_on_focus_change
                .unwrap_or(defaults.capture_on_focus_change),
            capture_on_window_change: local_history
                .capture_on_window_change
                .unwrap_or(defaults.capture_on_window_change),
            capture_on_task: local_history
                .capture_on_task
                .unwrap_or(defaults.capture_on_task),
            ...
            min_age_days: local_history.min_age_days.unwrap_or(defaults.min_age_days),
            cap_free_space_percent: local_history
                .cap_free_space_percent
                .unwrap_or(defaults.cap_free_space_percent),
            cap_min_bytes: local_history
                .cap_min_bytes
                .unwrap_or(defaults.cap_min_bytes),
            prune_policy: local_history.prune_policy.unwrap_or(defaults.prune_policy),
            exclude_globs,
        }
    }
}
...
impl Default for LocalHistorySettings {
    fn default() -> Self {
        Self {
            enabled: true,
            capture_on_save: false,
            capture_on_edit_idle_ms: Some(DEFAULT_EDIT_IDLE_MS),
            capture_on_focus_change: true,
            capture_on_window_change: true,
            capture_on_task: true,
            capture_on_external_change: true,
            storage_paths: Vec::new(),
            active_storage_path: None,
            min_age_days: DEFAULT_MIN_AGE_DAYS,
            cap_free_space_percent: DEFAULT_FREE_SPACE_PERCENT,
            cap_min_bytes: DEFAULT_MIN_CAP_BYTES,
...
impl LocalHistoryStore {
    ...
    pub fn load_entry_text(&self, entry: LocalHistoryEntry, cx: &App) -> Task<Result<Arc<str>>> {
        cx.background_spawn(async move {
            ...
            let Some(worktree_hash) = extract_worktree_hash(&entry.snapshot_relative_path) else {
                return load_snapshot_text(&path);
            };
            ...
            let index_path = entry
                ...
                .join(&worktree_hash)
                .join(INDEX_FILE_NAME);
            let records = match read_index(&index_path) {
                Ok(records) => records,
                Err(_) => return load_snapshot_text(&path),
            };
            let records_by_id = records_by_id(&records);
            let Some(record) = records_by_id.get(&entry.id) else {
                return load_snapshot_text(&path);
            };

            let mut text_cache = HashMap::default();
            load_record_text(
                &entry.endpoint_root,
                &worktree_hash,
                &records_by_id,
    ...
    pub fn transfer_history(
        ...
        cx.background_spawn(async move { transfer_local_history(&source, &destination, mode) })
    }

    pub fn set_entry_bookmarked(
        &mut self,
        ...
        cx.spawn(async move |_, cx| {
            ...
            if let Err(err) = result {
                log::warn!("local history bookmark update failed: {err:#}");
            }
...
pub enum LocalHistoryStorageKind {
    Snapshot,
    Delta,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
struct LocalHistoryDeltaEdit {
    old_start: usize,
    old_end: usize,
    new_text: String,
}
...
#[derive(Clone, Debug, Serialize, Deserialize)]
struct LocalHistoryDeltaPayload {
    edits: Vec<LocalHistoryDeltaEdit>,
}
...
struct LocalHistoryEntryRecord {
    ...
    storage_kind: Option<LocalHistoryStorageKind>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    base_entry_id: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    chain_depth: Option<u16>,
}
...
fn write_snapshot(
    ...
    let existing_records = if index_path.exists() {
        read_index(&index_path)?
    } else {
        Vec::new()
    };
    let previous_record = existing_records
        .iter()
        .rev()
        .find(|record| record.relative_path == relative_path)
        .cloned();
    let records_by_id = records_by_id(&existing_records);
    let mut text_cache = HashMap::default();
    let previous_text = previous_record
        .as_ref()
        .map(|record| {
            load_record_text(
                root,
    ...
    let record = write_record_payload(
        root,
        &worktree_hash,
        &entry_id,
        timestamp,
        relative_path,
        text,
        previous_record.as_ref().zip(previous_text.as_deref()),
        false,
    )?;
...
fn records_by_id(entries: &[LocalHistoryEntryRecord]) -> HashMap<String, LocalHistoryEntryRecord> {
    entries
        .iter()
        .cloned()
...
fn write_record_payload(
    ...
    relative_path: &str,
    text: &str,
    previous: Option<(&LocalHistoryEntryRecord, &str)>,
    bookmarked: bool,
) -> Result<LocalHistoryEntryRecord> {
    ...
    let storage = prepare_record_storage(text, previous)?;
    let snapshot_file_name = format!("{entry_id}.zst");
    let snapshot_relative_path = PathBuf::from(WORKTREES_DIR)
        .join(worktree_hash)
        .join(SNAPSHOTS_DIR_NAME)
        .join(snapshot_file_name);
    let snapshot_file_path = root.join(&snapshot_relative_path);
    fs::write(&snapshot_file_path, &storage.compressed)
        .with_context(|| format!("writing {:?}", snapshot_file_path))?;

    Ok(LocalHistoryEntryRecord {
        id: entry_id.to_string(),
        timestamp: timestamp.unix_timestamp(),
        relative_path: relative_path.to_string(),
        snapshot_path: snapshot_relative_path.to_string_lossy().to_string(),
        compressed_bytes: storage.compressed.len() as u64,
        uncompressed_bytes: text.len() as u64,
        bookmarked,
        storage_kind: Some(storage.kind),
        base_entry_id: storage.base_entry_id,
        chain_depth: Some(storage.chain_depth),
    })
}

struct PreparedRecordStorage {
    compressed: Vec<u8>,
    kind: LocalHistoryStorageKind,
    base_entry_id: Option<String>,
    chain_depth: u16,
}
...
fn prepare_record_storage(
    text: &str,
    previous: Option<(&LocalHistoryEntryRecord, &str)>,
) -> Result<PreparedRecordStorage> {
    let snapshot = encode_snapshot_payload(text)?;
    let mut storage = PreparedRecordStorage {
        compressed: snapshot,
        kind: LocalHistoryStorageKind::Snapshot,
        base_entry_id: None,
        chain_depth: 0,
    };

    let Some((previous_record, previous_text)) = previous else {
        return Ok(storage);
    };

    if record_chain_depth(previous_record) >= MAX_DELTA_CHAIN_DEPTH {
        return Ok(storage);
    }

    let delta = LocalHistoryDeltaPayload {
        edits: text_diff(previous_text, text)
            .into_iter()
            .map(|(range, replacement)| LocalHistoryDeltaEdit {
    ...
    let delta = encode_delta_payload(&delta)?;
    if delta.len() >= storage.compressed.len() {
        return Ok(storage);
    }

    storage.compressed = delta;
    storage.kind = LocalHistoryStorageKind::Delta;
    storage.base_entry_id = Some(previous_record.id.clone());
    storage.chain_depth = record_chain_depth(previous_record) + 1;
    Ok(storage)
}

fn encode_snapshot_payload(text: &str) -> Result<Vec<u8>> {
    Ok(zstd::encode_all(text.as_bytes(), 0)?)
}

fn encode_delta_payload(payload: &LocalHistoryDeltaPayload) -> Result<Vec<u8>> {
    let encoded = serde_json::to_vec(payload)?;
    Ok(zstd::encode_all(&encoded[..], 0)?)
}

fn load_snapshot_text(path: &Path) -> Result<Arc<str>> {
...
fn load_delta_payload(path: &Path) -> Result<LocalHistoryDeltaPayload> {
    let data = fs::read(path).with_context(|| format!("reading {:?}", path))?;
    let decoded = zstd::decode_all(&data[..])?;
...
fn load_record_text(
    root: &Path,
    worktree_hash: &str,
    records_by_id: &HashMap<String, LocalHistoryEntryRecord>,
    record: &LocalHistoryEntryRecord,
    text_cache: &mut HashMap<String, Arc<str>>,
) -> Result<Arc<str>> {
    if let Some(text) = text_cache.get(&record.id) {
        return Ok(text.clone());
    }

    let path = root.join(resolve_snapshot_relative_path(worktree_hash, record));
    let text = match record_storage_kind(record) {
        LocalHistoryStorageKind::Snapshot => load_snapshot_text(&path)?,
        LocalHistoryStorageKind::Delta => {
            let base_entry_id = record
                .base_entry_id
                .as_ref()
                .context("local history delta is missing its base entry")?;
            let base_record = records_by_id
                .get(base_entry_id)
                .with_context(|| format!("missing local history base entry {base_entry_id}"))?;
            let base_text =
                load_record_text(root, worktree_hash, records_by_id, base_record, text_cache)?;
            let delta = load_delta_payload(&path)?;
            Arc::<str>::from(apply_delta(&base_text, &delta)?)
        }
    };

    text_cache.insert(record.id.clone(), text.clone());
    Ok(text)
}

fn apply_delta(base_text: &str, payload: &LocalHistoryDeltaPayload) -> Result<String> {
    let mut result = String::with_capacity(base_text.len());
    let mut cursor = 0usize;

    for edit in &payload.edits {
        ensure!(
            edit.old_start <= edit.old_end,
            "invalid local history delta range {}..{}",
            edit.old_start,
            edit.old_end
        );
        ensure!(
            edit.old_start >= cursor,
            "overlapping local history delta range {}..{}",
            edit.old_start,
            edit.old_end
        );
        ensure!(
            edit.old_end <= base_text.len(),
            "local history delta range {}..{} exceeds base text length {}",
            edit.old_start,
            edit.old_end,
            base_text.len()
        );
        ensure!(
            base_text.is_char_boundary(edit.old_start) && base_text.is_char_boundary(edit.old_end),
            "local history delta range {}..{} is not on UTF-8 boundaries",
            edit.old_start,
            edit.old_end
        );

        result.push_str(&base_text[cursor..edit.old_start]);
        result.push_str(&edit.new_text);
        cursor = edit.old_end;
    }

    result.push_str(&base_text[cursor..]);
    Ok(result)
}

fn resolve_snapshot_relative_path(
    worktree_hash: &str,
    record: &LocalHistoryEntryRecord,
) -> PathBuf {
    if record.snapshot_path.starts_with(WORKTREES_DIR) {
...
fn resolve_snapshot_path(
    ...
    root.join(resolve_snapshot_relative_path(worktree_hash, record))
}

fn extract_worktree_hash(snapshot_relative_path: &Path) -> Option<String> {
    let mut components = snapshot_relative_path.iter();
    if components.next()? != std::ffi::OsStr::new(WORKTREES_DIR) {
        return None;
    }
    Some(components.next()?.to_string_lossy().to_string())
}

fn record_storage_kind(record: &LocalHistoryEntryRecord) -> LocalHistoryStorageKind {
    record
        .storage_kind
        .unwrap_or(LocalHistoryStorageKind::Snapshot)
}

fn record_chain_depth(record: &LocalHistoryEntryRecord) -> u16 {
    record.chain_depth.unwrap_or(0)
}
...
fn load_entries_from_endpoint(
    ...
    for line in reader.lines().flatten() {
        ...
        let storage_kind = record_storage_kind(&record);
        let chain_depth = record_chain_depth(&record);
        entries.push(LocalHistoryEntry {
            ...
            storage_kind,
            base_entry_id: record.base_entry_id,
            chain_depth,
        });
    }
    entries
...
fn prune_worktree_if_needed_at(
    ...
    let removed_ids: HashSet<_> = removed.into_iter().collect();
    let mut remaining: Vec<_> = entries
        .iter()
        .filter(|entry| !removed_ids.contains(&entry.id))
        .cloned()
        .collect();
    rebuild_records_after_deletions(root, &worktree_hash, &entries, &mut remaining, &removed_ids)?;
    ...
    for entry in &entries {
        ...
        let snapshot_path = resolve_snapshot_path(root, &worktree_hash, entry);
...
fn rebuild_records_after_deletions(
    root: &Path,
    worktree_hash: &str,
    original_records: &[LocalHistoryEntryRecord],
    remaining_records: &mut [LocalHistoryEntryRecord],
    removed_ids: &HashSet<String>,
) -> Result<()> {
    let original_by_id = records_by_id(original_records);
    let remaining_ix_by_id = remaining_records
        .iter()
        .enumerate()
        .map(|(ix, record)| (record.id.clone(), ix))
        .collect::<HashMap<_, _>>();
    let mut text_cache = HashMap::default();
    let mut records_by_path = HashMap::<String, Vec<&LocalHistoryEntryRecord>>::default();
    ...
    for path_records in records_by_path.values() {
        if !path_records
            .iter()
            .any(|record| removed_ids.contains(&record.id))
        {
            continue;
        }

        let mut should_rebuild = false;
        let mut previous_survivor_id: Option<String> = None;
        for record in path_records {
            ...
            let Some(&remaining_ix) = remaining_ix_by_id.get(&record.id) else {
                continue;
            };

            if !should_rebuild {
                previous_survivor_id = Some(record.id.clone());
                continue;
            }

            let current_text = load_record_text(
                root,
                worktree_hash,
                &original_by_id,
                original_by_id
                    .get(&record.id)
                    .with_context(|| format!("missing local history entry {}", record.id))?,
                &mut text_cache,
            )?;
            let previous_record = previous_survivor_id
                .as_ref()
                .and_then(|id| remaining_ix_by_id.get(id))
                .map(|ix| remaining_records[*ix].clone());
            let previous_text = previous_survivor_id
                .as_ref()
                .map(|id| {
                    let record = original_by_id
                        .get(id)
                        .with_context(|| format!("missing local history entry {id}"))?;
                    load_record_text(
                        root,
                        worktree_hash,
                        &original_by_id,
                ...
                .transpose()?;
            let timestamp = OffsetDateTime::from_unix_timestamp(record.timestamp)?;
            let rebuilt = write_record_payload(
                root,
                worktree_hash,
                &record.id,
                timestamp,
            ...
            remaining_records[remaining_ix] = rebuilt;
            previous_survivor_id = Some(record.id.clone());
        }
    }
...
pub fn load_local_history_entries(
    ...
    let mut entries = roots
        .into_iter()
        .flat_map(|root| load_entries_from_endpoint(&root, worktree_path, relative_path))
        .collect::<Vec<_>>();
    entries.sort_by(|a, b| b.timestamp.cmp(&a.timestamp));
    entries
}

pub fn load_local_history_entry_text(entry: &LocalHistoryEntry) -> Result<Arc<str>> {
    let path = entry.snapshot_path();
    let Some(worktree_hash) = entry.worktree_hash() else {
        return load_snapshot_text(&path);
    };
...
pub fn set_local_history_entry_bookmarked(
    entry: &LocalHistoryEntry,
    bookmarked: bool,
) -> Result<bool> {
    let index_path = entry
        .index_path()
        .context("local history entry is missing its index path")?;
    let mut updated = false;
    let modified = rewrite_index_records(&index_path, |record| {
        if record.id != entry.id {
            return false;
        }
        updated = record.bookmarked != bookmarked;
        record.bookmarked = bookmarked;
        true
    })?;
    ensure!(modified > 0, "local history entry {} not found", entry.id);
    Ok(updated)
}

pub fn transfer_local_history(
    source: &Path,
    destination: &Path,
    mode: LocalHistoryTransferMode,
) -> Result<()> {
...
pub fn prune_local_history_worktree(
    root: &Path,
    worktree_path: &Path,
    settings: &LocalHistorySettings,
) -> Result<usize> {
    prune_worktree_if_needed(root, worktree_path, settings)
}
...
mod tests {
    ...
    fn load_text_for_entry(root: &Path, worktree_path: &Path, entry_id: &str) -> Arc<str> {
        let worktree_hash = hash_worktree_path(worktree_path);
        let index_path = root
            .join(WORKTREES_DIR)
            .join(&worktree_hash)
            .join(INDEX_FILE_NAME);
        let records = read_index(&index_path).unwrap();
        let records_by_id = records_by_id(&records);
        let mut text_cache = HashMap::default();
        load_record_text(
            root,
            &worktree_hash,
            &records_by_id,
            records_by_id.get(entry_id).unwrap(),
            &mut text_cache,
        )
        .unwrap()
    }
    ...
    fn prune_policy_both_requires_age_and_size() {
        ...
        for (id, ts, size) in [("old", old_ts, 600_u64), ("new", new_ts, 600_u64)] {
            ...
            std::fs::write(&snapshot_path, encode_snapshot_payload("").unwrap()).unwrap();
            let record = LocalHistoryEntryRecord {
                id: id.to_string(),
    ...
    fn write_snapshot_roundtrip_preserves_text() {
        ...
        assert_eq!(record.storage_kind, Some(LocalHistoryStorageKind::Snapshot));
    ...
    #[test]
    fn bookmark_toggle_roundtrips_through_index() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let worktree_path = Path::new("/test/worktree");
        let text = "hello bookmark\n";
        let timestamp = OffsetDateTime::from_unix_timestamp(1_700_000_124).unwrap();

        write_snapshot(root, worktree_path, "file.txt", text, timestamp).unwrap();

        let entries = load_entries_from_endpoint(root, worktree_path, "file.txt");
        assert_eq!(entries.len(), 1);
        assert!(!entries[0].bookmarked);

        set_local_history_entry_bookmarked(&entries[0], true).unwrap();
        let entries = load_entries_from_endpoint(root, worktree_path, "file.txt");
        assert!(entries[0].bookmarked);

        set_local_history_entry_bookmarked(&entries[0], false).unwrap();
        let entries = load_entries_from_endpoint(root, worktree_path, "file.txt");
        assert!(!entries[0].bookmarked);
    }

    #[test]
    fn write_snapshot_uses_delta_when_it_is_smaller() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let worktree_path = Path::new("/test/worktree");
        let base = (0..160)
            .map(|ix| format!("line {ix:03}: keep this stable for delta compression\n"))
            .collect::<String>();
        let updated = format!("{base}tail edit\n");
        ...
        let second = write_snapshot(
            ...
            "file.txt",
            &updated,
            OffsetDateTime::from_unix_timestamp(1_700_000_101).unwrap(),
        )
        .unwrap();

        assert_eq!(first.storage_kind, Some(LocalHistoryStorageKind::Snapshot));
        assert_eq!(second.storage_kind, Some(LocalHistoryStorageKind::Delta));
        assert_eq!(second.base_entry_id.as_deref(), Some(first.id.as_str()));
        assert_eq!(second.chain_depth, Some(1));
        assert_eq!(
            load_text_for_entry(root, worktree_path, &second.id).as_ref(),
            updated
        );
    }

    #[test]
    fn rebuild_records_after_middle_deletion_keeps_latest_entry_loadable() {
        let temp = tempdir().unwrap();
        let root = temp.path();
        let worktree_path = Path::new("/test/worktree");
        let worktree_hash = hash_worktree_path(worktree_path);
        let base = (0..120)
            .map(|ix| format!("line {ix:03}: stable text\n"))
            .collect::<String>();
        let middle = base.replacen("line 060: stable text\n", "line 060: middle edit\n", 1);
        let latest = middle.replacen("line 090: stable text\n", "line 090: latest edit\n", 1);
        ...
        .unwrap();
        let middle_record = write_snapshot(
            root,
            worktree_path,
            "file.txt",
            &middle,
            OffsetDateTime::from_unix_timestamp(1_700_000_201).unwrap(),
        )
        .unwrap();
        let latest_record = write_snapshot(
            root,
            worktree_path,
            "file.txt",
            &latest,
            OffsetDateTime::from_unix_timestamp(1_700_000_202).unwrap(),
        )
        .unwrap();
        ...
        let index_path = root
            ...
            .join(&worktree_hash)
            .join(INDEX_FILE_NAME);
        let original = read_index(&index_path).unwrap();
        let removed_ids = [middle_record.id.clone()]
            .into_iter()
            .collect::<HashSet<_>>();
        let mut remaining = original
            ...
            .cloned()
            .collect::<Vec<_>>();
        rebuild_records_after_deletions(
            root,
            &worktree_hash,
            &original,
            &mut remaining,
        ...
        .unwrap();
        write_index(&index_path, &remaining).unwrap();

        std::fs::remove_file(resolve_snapshot_path(root, &worktree_hash, &middle_record)).unwrap();

        let latest_remaining = remaining
            .iter()
            .find(|record| record.id == latest_record.id)
            .unwrap();
        assert_ne!(
            latest_remaining.base_entry_id.as_deref(),
            Some(middle_record.id.as_str())
        );
        assert_eq!(
            load_text_for_entry(root, worktree_path, &latest_record.id).as_ref(),
            latest
        );
        assert_eq!(
            load_text_for_entry(root, worktree_path, &first.id).as_ref(),
            base
        );
    }
